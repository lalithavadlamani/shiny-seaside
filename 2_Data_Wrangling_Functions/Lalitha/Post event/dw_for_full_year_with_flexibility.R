...
full_year_analysis <- function(list_of_df){
  ...
  final_df[1,'learning_kpi'] <- learning_kpi(data)
  ...
  final_df[1,'community_kpi'] <- community_kpi(data)
  ...
  final_df[1,'action_doing_before_the_event'] <- doing_before_per
  final_df[1,'action_doing_after_the_event'] <- doing_after_per
  final_df[1,'action_likely_to_do'] <- likely_do_per
  final_df[1,'action_unlikely_to_do'] <- unlikely_do_per
  
  final_df[1,'env_habits'] <- mean(data$on.a.scale.of.1.5..how.would.you.rate.your.environmental.habits.after.the.activity.,na.rm = TRUE)
