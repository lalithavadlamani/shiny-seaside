...
PreEventPlot <- function(data, var1, var2, title, varNum) {
  
  
  data = data %>% dplyr::rename(var1 = var1, var2 = var2)
  if(varNum == 1) {
    ...
    data %>% ggplot(aes(x = var1, fill = var2)) + 
      geom_bar() + 
      ggtitle(title) + 
...
PreEventPlot(data, "previous_attendance", "pronoun", 'Previous Event Attendance by Gender', 2)
