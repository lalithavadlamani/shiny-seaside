...
PreEventPlot <- function(data, varNames, title) {
  ...
  if(length(varNames) == 1) {
    ...
    data %>% group_by(var1) %>% dplyr::count() %>% ggplot(aes(x = reorder(varViz, varViz, function(x)-length(x)))) + geom_bar()
...
PreEventPlot(data, c("previous_attendance", "pronoun"), 'Previous Event Attendance by Gender')
