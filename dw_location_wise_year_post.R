...
location_wise_analysis_for_a_year <- function(list_of_df){
  ...
  preprocess <- function(data){
    df <- data[-c(1,12:21)] 
    colnames(df) <- c('first_activity','takeaway','use_hand2','use_lp','compost','drop_waste','contact_council','use_skills','env_habits','channel_preferred','location')
    return(df)
  }
  
  df <- preprocess(data)
  
  analysis <- function(loc_data){
    ...
    first_activity_table <- prop.table(FA(loc_data$first_activity))
    ...
    biggest_takeaway_kpi_table <- count_takeaways(loc_data$takeaway)
    ...
    env_habits_table <- prop.table(env_habits_scale(loc_data$env_habits))
    ...
    preferred_channel_table <- prop.table(channel(loc_data$channel_preferred))
    
    # Function for Action kPI 
    action_kpi <- function(loc_data){
      uh2 <- table(loc_data$use_hand2)
      ulp <- table(loc_data$use_lp)
      cp <- table(loc_data$compost)
      dw <- table(loc_data$drop_waste)
      cc <- table(loc_data$contact_council)
      us <- table(loc_data$use_skills)
  ...
  unique_locations <- unique(df[c('location')])
  ...
  for (i in 1:no_of_unique_locations){
    ...
    data_required <- subset(df ,location == location_name)
    
    loc_analysis <- analysis(data_required)
...
location_wise_analysis_for_a_year(list_of_df) # returns list of lists (essentially for each location it will return a list of tables)
