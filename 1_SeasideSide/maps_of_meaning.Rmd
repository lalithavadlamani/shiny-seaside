...
post$postcode_event = as.character(post$postcode_event)
post$`...1` = seq(nrow(post)) 
post_joined <- left_join(post, postcodes_loc, by = c("postcode_event" = "postcode"))
post_joined <- post_joined %>% dplyr::distinct(...1, .keep_all = TRUE)
post_joined <- post_joined %>%
                dplyr::rename(action_kpi = action_summary)

post_joined %>% dplyr::group_by(postcode_event, lat, long, locality) %>%
  dplyr::summarise(
    action_kpi = mean(action_kpi),
    count = n()
  )
```
...
```{r}
map_kpi(post, "action")
map_kpi(post, "community")
map_kpi(post, "learning")
```
